lexicase-selection(ns clojush.pushgp.selection.knobelty
  (:use [clojush random util globals])
  (:require [clojure.math.numeric-tower :as math]))

  (defn shuffle-cases-copy
    [pop argmap]
  ...
  (defn lexicase-selection-copy
    "Returns an individual that does the best on the fitness cases when considered one at a
    time in random order."
    [pop argmap]
    (loop [survivors pop
           cases (shuffle-cases-copy pop argmap)]
      (if (or (empty? cases)
  ...
  (defn select-individuals-for-novelty-archive-copy
  ...
  (defn calculate-behavior-distance-map-copy
                   ...
                   (map (fn [other-behavior]
                          (vector other-behavior
                                  (if (contains? behavior-distance-map other-behavior)
                                    ...
                                    (behavioral-distance-copy behavior other-behavior argmap))))
  ...
  (defn calculate-behavior-sparseness-copy
  ...
  (defn assign-novelty-to-individual-copy
  ...
  (defn calculate-novelty-copy
   ...
   (let [pop-behaviors (map #(:behaviors (deref %)) pop-agents)
         ...
         behavior-distance-map (calculate-behavior-distance-map-copy (distinct pop-behaviors)
                                                                (distinct pop-and-archive-behaviors)
                                                                argmap)
         behavior-sparseness (calculate-behavior-sparseness-copy pop-and-archive-behaviors
                                                            behavior-distance-map
                                                            argmap)]
     (dorun (map #((if use-single-thread swap! send)
                   % assign-novelty-to-individual-copy behavior-sparseness)
                 pop-agents)))
   (when-not use-single-thread (apply await pop-agents)) ;; SYNCHRONIZE
   (println "Done calculating novelty.")
   (println "\nNovelty Numbers:" (sort > (map #(float (:novelty (deref %))) pop-agents))))

  (defn novelty-tournament-selection-copy
  ...
  (defn assign-novelty-by-case-to-individual-copy
   ...
   (let [behaviors (:behaviors ind)
         ...
         novelty-by-case (map novelty-difference-copy ;#(novelty-difference %1 %2)
                                      behaviors
                                      case-behavior-vector)]
     (assoc ind :novelty-by-case novelty-by-case)))

  (defn calculate-lex-novelty-copy
   ...
   (dorun (map #((if use-single-thread swap! send) %
                 assign-novelty-by-case-to-individual-copy
...
(defn knobelty-selection
  [pop argmap]
  (if (< (rand) (:knobelty-const argmap)) (lexicase-selection-copy pop argmap) (novelty-tournament-selection-copy pop argmap)))
